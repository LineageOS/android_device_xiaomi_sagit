...
KeyInverser::KeyInverser() {
    mHasKeyInverser = !access(kControlPath, F_OK);
}

// Methods from ::vendor::lineage::touch::V1_0::IKeyInverser follow.
Return<bool> KeyInverser::isEnabled() {
    std::string buf;

    if (!mHasKeyInverser) return false;
...
Return<bool> KeyInverser::setEnabled(bool enabled) {
    if (!mHasKeyInverser) return false;
